...
### 2026-04-28 - 修复日期格式和数据类型问题 ✅

**已完成**:

1. **统一日期格式** (v0.5.60)
   - 创建 `fix_date_format.py` 清理脚本
   - 清理 21,034 条带连字符的日期记录 (YYYY-MM-DD → YYYYMMDD)
   - 在 `data/fetcher.py` 中添加日期格式自动转换
   - 确保所有日期在保存前统一为 YYYYMMDD 格式

2. **修复日期类型错误** (v0.5.61)
   - 在日期格式转换前先转换为字符串类型
   - 修复 `.str` 访问器错误 ("Can only use .str accessor with string values!")
   - 支持 akshare 返回的整数日期格式 (YYYYMMDD)
...
**已知问题**:
- API 连接不稳定 (akshare 和 baostock)
- 数据更新需要等待 API 服务恢复

### 2026-04-22 - 优化 baostock 请求超时时间 ✅

**已完成**:

1. **优化 baostock 请求超时时间** (v0.5.46)
   - `data/fetcher.py`: baostock 请求超时从 300 秒（5 分钟）缩短为 60 秒（1 分钟）
   - 快速失败：超时后立即跳过，继续处理下一只股票
   - 提升效率：避免单只股票超时影响整体进度
   - 更新更快：数据更新速度明显提升
   - `utils/config.py`: 版本号更新为 v0.5.46
   - `README.md`: 文档更新

### 2026-04-22 - 添加 baostock 请求超时控制 ✅

**已完成**:

1. **添加 baostock 请求超时控制** (v0.5.45)
   - `data/fetcher.py`: 使用 threading 实现超时控制
   - baostock 请求添加超时机制（默认 5 分钟）
   - 避免请求无限等待导致应用卡死
   - 超时时记录警告并抛出异常
   - `utils/config.py`: 版本号更新为 v0.5.45

### 2026-04-22 - 扩展历史数据至11年 ✅

**已完成**:

1. **扩展历史数据至11年** (v0.5.44)
   - `config/data_config.json`: start_date 从 2023-01-01 改为 2015-01-01
   - `utils/config.py`: DEFAULT_DATA_START_DATE 更新为 2015-01-01
   - 提供更完整的历史数据用于长期预测训练
   - 版本号更新为 v0.5.44
   - `README.md`: 文档更新

### 2026-04-22 - 修复长期预测数据不足问题 ✅

**已完成**:

1. **修复长期预测数据不足问题** (v0.5.43)
   - `config/data_config.json`: start_date 从 2025-01-01 改为 2023-01-01
   - 提供3年历史数据，确保长期预测有足够数据（需要至少372天）
   - 解决长期模型 "No training data available" 错误
   - 版本号更新为 v0.5.43
   - `README.md`: 文档更新

**注意**: 需要清空数据库中的历史数据并重新获取

### 2026-04-20 - 修改为不复权价格存储 ✅

**已完成**:

1. **修改为不复权价格** (v0.5.41)
   - `data/fetcher.py`: 修改 akshare adjust="hfq" 为 adjust=""（不复权）
   - `data/fetcher.py`: 修改 baostock adjustflag="1" 为 adjustflag="3"（不复权）
   - 数据存储为实际市场价格，而非前复权价格
   - 招商银行等有分红历史的股票价格将显示实际价格
...
2. **板块和龙头股配置化** (v0.5.12)
   ...
   - 配置 29 个板块（24个行业 + 5个概念），每个板块 5 只股票
   - 总股票记录数 145 只，去重后 116 只唯一股票
   - 修改"更新股票数据"逻辑，只更新配置的股票
   - 添加 `get_major_sectors_config()` 方法读取配置
   - 后续技术指标更新和模型训练都基于配置的股票（去重后116只）
...
5. **技术指标和模型训练优化** (v0.5.15)
   - `更新技术指标`: 只处理配置的股票（去重后116只），不再处理数据库所有股票
   - `训练预测模型`: 只使用配置的股票训练
   - 修复"更新技术指标"界面卡死问题
   - 大幅提升性能：116只股票 vs 数千只
...
10. **技术指标更新逻辑优化** (v0.5.20)
    - 修复已有最新数据时的错误判断逻辑
    - 当 `start_date > end_date` 时，说明已有最新数据，标记为成功
    - 避免将"已有今日数据"错误地归类为"失败"
    - 减少失败统计，更准确反映更新状态
    - 清理重复的 import 语句
...
**Commits**:
- 4cf0f92: fix: add message for sectors without leaders
...
**已完成**:
1. **错误消息改进** - `ui/pages.py`
   - 将"跳过"改为"失败"，更准确反映状态
   - 添加失败原因追踪
   - 改进用户提示，指出 API 连接问题

**效果**:
- ✅ 用户现在看到"已处理: X 只 | 失败: Y 只"而不是"跳过"
- ✅ 当所有股票失败时，提示可能是 API 连接问题

**Commits**:
- 270c6c9: fix: improve stock update error messaging
